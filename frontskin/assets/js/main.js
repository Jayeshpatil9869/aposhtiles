...
// Explore Collection Tab Functionality
$(document).ready(function() {
    // Tab functionality
    $('#exploreTab a').on('click', function (e) {
        e.preventDefault();
        $(this).tab('show');
    });

    // Initialize the first tab
    $('#exploreTab a:first').tab('show');

    // Pagination dots functionality
    $('.explore-pagination .slick-dots li button').on('click', function() {
        var index = $(this).parent().index();
        $('.explore-pagination .slick-dots li').removeClass('slick-active');
        $(this).parent().addClass('slick-active');
        
        // Show corresponding tab content
        if (index === 0) {
            $('#exploreTab a[href="#locations"]').tab('show');
        } else if (index === 1) {
            $('#exploreTab a[href="#effects"]').tab('show');
        }
    });

    // Update pagination dots when tab changes
    $('#exploreTab a').on('shown.bs.tab', function (e) {
        var id = $(e.target).attr('href');
        if (id === '#locations') {
            $('.explore-pagination .slick-dots li').removeClass('slick-active');
            $('.explore-pagination .slick-dots li:first-child').addClass('slick-active');
        } else if (id === '#effects') {
            $('.explore-pagination .slick-dots li').removeClass('slick-active');
            $('.explore-pagination .slick-dots li:nth-child(2)').addClass('slick-active');
        } else if (id === '#colors') {
            $('.explore-pagination .slick-dots li').removeClass('slick-active');
            $('.explore-pagination .slick-dots li:first-child').addClass('slick-active');
        }
...
// Effect Slider Functionality
$(document).ready(function() {
    // Initialize effect slider
    var currentSlide = 0;
    var totalSlides = $('.effect-slide').length;
    
    // Show the first slide
    $('.effect-slide').hide();
    $('.effect-slide').eq(0).show();
    
    // Function to show a specific slide
    function showSlide(index) {
        $('.effect-slide').hide();
        $('.effect-slide').eq(index).fadeIn(500);
        
        // Update pagination dots
        $('.effect-pagination .slick-dots li').removeClass('slick-active');
        $('.effect-pagination .slick-dots li').eq(index).addClass('slick-active');
        
        currentSlide = index;
    }
    
    // Handle dot clicks
    $('.effect-pagination .slick-dots li').on('click', function() {
